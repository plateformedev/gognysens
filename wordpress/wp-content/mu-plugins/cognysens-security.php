<?php
/**
 * Plugin Name: Cognysens Security
 * Description: Must-use security plugin for Cognysens
 * Version: 1.0.0
 *
 * @package Cognysens
 */
 ...
 */
class Cognysens_Login_Security {
    ...
    public function check_attempts($user, $username, $password) {
        ...
        if ($attempts >= $this->max_attempts) {
            return new WP_Error(
                'too_many_attempts',
                sprintf(
                    __('Trop de tentatives de connexion. Reessayez dans %d minutes.', 'cognysens'),
...
Cognysens_Login_Security::get_instance();
...
add_filter('login_errors', function($error) {
    return __('Identifiants incorrects.', 'cognysens');
});
