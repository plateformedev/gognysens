...
class Cognysens_Booking_AI {
    ...
    private function __construct() {
        // Load settings
        $this->api_key = get_option('cognysens_ai_api_key', '');
        $this->api_endpoint = get_option('cognysens_ai_endpoint', '');

        // Hooks for booking integration
        add_action('cognysens_process_booking', array($this, 'process_booking'), 10, 2);
        add_filter('cognysens_booking_summary', array($this, 'generate_summary'), 10, 2);
    }
    ...
    public function process_booking($booking_data, $form_responses) {
        ...
        try {
            ...
            error_log('Cognysens AI Error: ' . $e->getMessage());
    ...
    private function build_prompt($data) {
        ...
        return <<<PROMPT
Tu es l'assistant de qualification de Cognysens, cabinet d'expertise en bati ancien.
...
PROMPT;
    ...
    public static function register_settings() {
        register_setting('cognysens_settings', 'cognysens_ai_api_key');
        register_setting('cognysens_settings', 'cognysens_ai_endpoint');

        add_settings_section(
            'cognysens_ai_section',
            __('Configuration IA', 'cognysens-core'),
            function() {
                echo '<p>' . esc_html__('Configurez l\'API IA pour la qualification automatique des demandes.', 'cognysens-core') . '</p>';
            },
            'cognysens_settings'
        );

        add_settings_field(
            'cognysens_ai_endpoint',
            __('Endpoint API', 'cognysens-core'),
            function() {
                $value = get_option('cognysens_ai_endpoint', '');
                echo '<input type="url" name="cognysens_ai_endpoint" value="' . esc_attr($value) . '" class="regular-text" />';
                echo '<p class="description">' . esc_html__('Ex: https://api.anthropic.com/v1/messages', 'cognysens-core') . '</p>';
            },
            'cognysens_settings',
            'cognysens_ai_section'
        );
        ...
        add_settings_field(
            'cognysens_ai_api_key',
            __('Cle API', 'cognysens-core'),
            function() {
                $value = get_option('cognysens_ai_api_key', '');
                echo '<input type="password" name="cognysens_ai_api_key" value="' . esc_attr($value) . '" class="regular-text" />';
            },
            'cognysens_settings',
            'cognysens_ai_section'
        );
