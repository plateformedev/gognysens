 ...
 */
function cognysens_complianz_custom_css() {
    if (!function_exists('cmplz_uses_optin')) {
        return;
    }
    ?>
    <style id="cognysens-complianz-css">
...
add_action('wp_head', 'cognysens_complianz_custom_css', 100);
 ...
 */
function cognysens_block_scripts_before_consent() {
    ...
    if (function_exists('cmplz_has_consent') && !cmplz_has_consent('statistics')) {
        ...
        remove_action('wp_head', 'cognysens_custom_analytics', 1);
    }
}
add_action('wp_head', 'cognysens_block_scripts_before_consent', 1);

/**
 * Filter Complianz settings (if available)
 */
function cognysens_complianz_settings($settings) {
...
add_filter('cmplz_settings', 'cognysens_complianz_settings');

/**
 * Admin notice if Complianz is not configured
 */
function cognysens_complianz_notice() {
    ...
    if (!function_exists('cmplz_uses_optin')) {
        ...
        <div class="notice notice-warning">
            <p><strong>Cognysens RGPD :</strong> Le plugin Complianz n'est pas installe.
...
add_action('admin_notices', 'cognysens_complianz_notice');
