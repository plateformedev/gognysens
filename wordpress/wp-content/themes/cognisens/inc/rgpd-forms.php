 ...
 */
function cognisens_rgpd_checkbox($args = array()) {
    $defaults = array(
        'name'     => 'rgpd_consent',
        'id'       => 'rgpd-consent',
        'required' => true,
        'class'    => 'cognisens-rgpd-checkbox',
    );
    ...
    <div class="<?php echo esc_attr($args['class']); ?>">
        <label class="rgpd-label">
            ...
            <span class="rgpd-text">
                ...
                de Cognisens. <?php echo $required_mark; ?>
            </span>
        </label>
    </div>
    <?php
 ...
 * @return string HTML checkbox
 */
function cognisens_rgpd_ai_checkbox($args = array()) {
    $defaults = array(
        'name'     => 'rgpd_ai_consent',
        'id'       => 'rgpd-ai-consent',
        'required' => true,
        'class'    => 'cognisens-rgpd-checkbox cognisens-rgpd-ai',
    );
 ...
 */
function cognisens_rgpd_checkbox_shortcode($atts) {
    ...
    return cognisens_rgpd_checkbox($atts);
}
add_shortcode('cognisens_rgpd', 'cognisens_rgpd_checkbox_shortcode');

/**
 * Shortcode for RGPD + AI checkbox
 */
function cognisens_rgpd_ai_shortcode($atts) {
    $atts = shortcode_atts(array(
        'name'     => 'rgpd_ai_consent',
        'id'       => 'rgpd-ai-consent',
        'required' => 'true',
    ...
    return cognisens_rgpd_ai_checkbox($atts);
}
add_shortcode('cognisens_rgpd_ai', 'cognisens_rgpd_ai_shortcode');

/**
 * Gravity Forms RGPD field filter
 */
function cognisens_gf_rgpd_field($field_content, $field, $value, $lead_id, $form_id) {
    if ($field->cssClass && strpos($field->cssClass, 'cognisens-rgpd') !== false) {
...
add_filter('gform_field_content', 'cognisens_gf_rgpd_field', 10, 5);

/**
 * Validate RGPD consent on form submission
 */
function cognisens_validate_rgpd_consent() {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        // Check if this is a form submission that needs RGPD validation
        if (isset($_POST['cognisens_form']) && !isset($_POST['rgpd_consent'])) {
            wp_die(
                __('Vous devez accepter la politique de confidentialite pour soumettre ce formulaire.', 'cognisens'),
                __('Consentement requis', 'cognisens'),
                array('back_link' => true)
            );
        }
    }
}
add_action('init', 'cognisens_validate_rgpd_consent');

/**
 * Log RGPD consent for compliance
 */
function cognisens_log_rgpd_consent($form_data) {
    ...
    $consent_log = array(
        ...
        'ip_address' => cognisens_get_client_ip(),
    ...
    $consents = get_option('cognisens_rgpd_consents', array());
    ...
    update_option('cognisens_rgpd_consents', $consents);
}

/**
 * Get client IP for consent logging
 */
function cognisens_get_client_ip() {
    $ip = '';
